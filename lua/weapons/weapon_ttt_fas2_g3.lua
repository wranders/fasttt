if GAMEMODE_NAME == "terrortown" then
	for _, wep in pairs( weapons.GetList() ) do
		if wep.ClassName == "fas2_g3" then
			AddCSLuaFile("weapons/fas2_g3/shared.lua")
			
			include("weapons/fas2_g3/shared.lua")
			...
			SWEP.EquipMenuData = {
				type = "Weapon",
				desc = "Heckler & Koch G3"
			};
			
			local enabled = GetConVar("ttt_fas2_wep_g3_enabled")
			local aquire = GetConVar("ttt_fas2_wep_g3_aquire")
			local available = GetConVar("ttt_fas2_wep_g3_available")
			local defaultfor = GetConVar("ttt_fas2_wep_g3_defaultfor")
			...
			if enabled:GetInt() == 1 then 
				...
				SWEP.Icon = "VGUI/entities/fas2_g3"
				if SERVER then
					resource.AddFile( "materials/VGUI/entities/fas2_g3.vmt" )
				end
