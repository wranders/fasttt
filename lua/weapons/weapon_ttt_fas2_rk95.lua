if GAMEMODE_NAME == "terrortown" then
	for _, wep in pairs( weapons.GetList() ) do
		if wep.ClassName == "fas2_rk95" then
			AddCSLuaFile("weapons/fas2_rk95/shared.lua")
			
			include("weapons/fas2_rk95/shared.lua")
			...
			local enabled = GetConVar("ttt_fas2_wep_rk95_enabled")
			local aquire = GetConVar("ttt_fas2_wep_rk95_aquire")
			local available = GetConVar("ttt_fas2_wep_rk95_available")
			local defaultfor = GetConVar("ttt_fas2_wep_rk95_defaultfor")
			...
			if enabled:GetInt() == 1 then 
				...
				SWEP.Icon = "VGUI/entities/fas2_rk95"
				if SERVER then
					resource.AddFile( "materials/VGUI/entities/fas2_rk95.vmt" )
				end 
